    ...
    namespace Algorithms
    {
            ...
            else if (graph.isWeighted()) // Dijkstra's algorithm
            {
                ...
                else
                {
                    ...
                    while (current != -1)
                    {
                        ...
                        if (current == src) break;  // Once we reach the source, stop.
        ...
        bool isContainsCycle(Graph& graph) 
        {
            ...
            if (hasCycle) 
            {
                ...
                cyclePath.pop_back(); cyclePath.pop_back(); // Remove the last " -> "
       ...
       string negativeCycle(Graph graph)
        {
            // Check for the presence of vertices
            if (graph.getVerticesCount() == 0)
            {
            ...
            vector<int> dist(graph.getVerticesCount(), 0); // Initialize all distances to 0

            for (int i = 0; i < graph.getVerticesCount() - 1; i++) // Relax all edges V-1 times
            {
                for (size_t u = 0; u < graph.getVerticesCount(); u++) 
                {
                    vector<int> neighbors = graph.getNeighbors(u);
                    for (int neighbor : neighbors) 
                    {
            ...
            // Check for negative cycles - if any distance can be further relaxed, then there is a negative cycle
            for (size_t u = 0; u < graph.getVerticesCount(); u++) 
            {
                vector<int> neighbors = graph.getNeighbors(u);
                for (int neighbor : neighbors) 
